...
#include<vector>
#include<chrono>
#include "my_lirpa_preload_fix.cu"
#include<fstream>
...
void pre_upload_weights_to_gpu(FullyConnectedNetwork* net) {
    for (int i = 0; i < net->num_layers; ++i) {
        cudaMalloc(&net->d_W[i], sizeof(Matrix));  // Matrix 전체 할당
        cudaMemcpy(net->d_W[i], &net->W[i], sizeof(Matrix), cudaMemcpyHostToDevice);
    }
}
...
int main(int argc, char** argv){
    ...
    std::cout << "  ** MODE: MEMPOOL + Weights PRELOAD ON GPU (FIXED)**" << std::endl;
    std::cout << "========================================\n";
    ...
    std::cout << "calc success (100 iterations)" << std::endl;
