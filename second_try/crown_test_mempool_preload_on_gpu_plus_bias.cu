...
#include<vector>
#include<chrono>
#include "my_lirpa_mempool_preload_on_gpu_plus_bias.cu"
#include<fstream>
...
// bias GPU에 사전 로드
void pre_upload_bias_to_gpu(FullyConnectedNetwork* net) {
    for (int i = 0; i < net->num_layers; ++i) {
        cudaMalloc(&net->d_b[i], MAX_DIM * sizeof(double));
        cudaMemcpy(net->d_b[i], net->b[i].v, MAX_DIM * sizeof(double), cudaMemcpyHostToDevice);
    }
}

void free_bias_from_gpu(FullyConnectedNetwork* net) {
    for (int i = 0; i < net->num_layers; ++i) {
        if (net->d_b[i]) cudaFree(net->d_b[i]);
    }
}
...
int main(int argc, char** argv){
    ...
    std::cout << "  ** MODE: MEMPOOL + Weights, bias PRELOAD ON GPU **" << std::endl;
    std::cout << "========================================\n";
