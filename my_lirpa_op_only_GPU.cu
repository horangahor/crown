// ============================================================
// my_lirpa_op.cu - 최적화된 CROWN/LiRPA 구현
// 
// my_lirpa.cu 대비 최적화 포인트:
//   1. cuBLAS 사용 (matmul) - naive GPU 커널 대비 수십 배 빠른 행렬곱
//   2. GPU 메모리 풀 - cudaMalloc/cudaFree 반복 호출 제거
//   3. 연산 크기별 CPU/GPU 분리:
//      - O(n³) 연산 (matmul) → GPU (cuBLAS)
//      - O(n²) 행렬 연산 (mat_add 등) → GPU (풀 사용)
//      - O(n) 벡터 연산 (vec_add 등) → CPU (GPU 오버헤드 > 연산 비용)
//
// 컴파일: nvcc -Xcompiler "/utf-8" -Xlinker "/STACK:134217728"
//         -arch=sm_89 -lcublas crown_test_op.cu -o crown_test_op.exe
// ============================================================
...
// ============================================================
// 2. GPU 메모리 풀 (프로그램 시작 시 1회 할당, 종료 시 해제)
//    - cudaMalloc/cudaFree 반복 호출을 완전히 제거
//    - 모든 GPU 함수는 이 풀에서 버퍼를 가져다 씀
// ============================================================

constexpr int NUM_GPU_MAT = 4; // matmul(3) + 여유(1)
constexpr int NUM_GPU_VEC = 2; // 여유

struct GpuPool {
  Matrix* d_mat[NUM_GPU_MAT];
  Vector* d_vec[NUM_GPU_VEC];
  cublasHandle_t cublas;
  bool initialized = false;

  void init() {
    if (initialized) return;
    for (int i = 0; i < NUM_GPU_MAT; i++)
      cudaMalloc(&d_mat[i], sizeof(Matrix));
    for (int i = 0; i < NUM_GPU_VEC; i++)
...
// ============================================================
// 4. CPU 함수: 행렬/벡터 생성 (GPU로 보낼 필요 없음)
// ============================================================
...
__global__ void mat_add_kernel(const Matrix *A, const Matrix *B, Matrix *C) {
...
__global__ void positive_part_kernel(const Matrix *A, Matrix *out) {
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  int total = A->rows * A->cols;
  if (tid < total) {
    int i = tid / A->cols;
    int j = tid % A->cols;
    double val = A->a[i][j];
    out->a[i][j] = (val > 0.0) ? val : 0.0;
  }
}

__global__ void negative_part_kernel(const Matrix *A, Matrix *out) {
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  int total = A->rows * A->cols;
  if (tid < total) {
    int i = tid / A->cols;
...
__global__ void rowwise_scale_kernel(const Matrix *A, const Vector *s, Matrix *out) {
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  int total = A->rows * A->cols;
  if (tid < total) {
    int i = tid / A->cols;
...
// --- GPU 행렬 연산 (풀 사용, cudaMalloc/Free 제거) ---
...
Matrix mat_add(const Matrix &A, const Matrix &B) {
  ...
  mat_add_kernel<<<blocks, threads>>>(g_pool.d_mat[0], g_pool.d_mat[1], g_pool.d_mat[2]);
...
Matrix positive_part(const Matrix &A) {
  ...
  int threads = 256;
  int blocks = (total + threads - 1) / threads;
  positive_part_kernel<<<blocks, threads>>>(g_pool.d_mat[0], g_pool.d_mat[1]);
  cudaDeviceSynchronize();
...
Matrix negative_part(const Matrix &A) {
  ...
  int threads = 256;
  int blocks = (total + threads - 1) / threads;
  negative_part_kernel<<<blocks, threads>>>(g_pool.d_mat[0], g_pool.d_mat[1]);
  cudaDeviceSynchronize();
...
Matrix rowwise_scale(const Matrix &A, const Vector &s) {
  ...
  int threads = 256;
  int blocks = (total + threads - 1) / threads;
  rowwise_scale_kernel<<<blocks, threads>>>(g_pool.d_mat[0], g_pool.d_vec[0], g_pool.d_mat[1]);
  cudaDeviceSynchronize();
...
// --- cuBLAS 행렬곱 (핵심 최적화!) ---
// naive GPU 커널 대비 수십 배 빠른 NVIDIA 최적화 행렬곱
// Row-major → Column-major 변환 트릭:
//   C_row = A_row * B_row  ↔  C^T = B^T * A^T (column-major)
//   cuBLAS는 column-major이므로, row-major 데이터를 넣으면 자동으로 전치됨
//   → cublasDgemm(B^T, A^T) 순서로 호출하면 결과가 C_row에 맞게 나옴
...
Matrix matmul(const Matrix &A, const Matrix &B) {
  ...
  // Matrix 구조체 내 double 배열의 오프셋 계산
  size_t a_offset = offsetof(Matrix, a);
  double* d_A = (double*)((char*)g_pool.d_mat[0] + a_offset);
  double* d_B = (double*)((char*)g_pool.d_mat[1] + a_offset);
  double* d_C = (double*)((char*)g_pool.d_mat[2] + a_offset);

  double alpha = 1.0, beta = 0.0;

  // Row-major 트릭: C^T = B^T * A^T
  // cublasDgemm(handle, opA, opB, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc)
  // 여기서 cuBLAS의 A = 우리의 B^T, cuBLAS의 B = 우리의 A^T
  cublasDgemm(g_pool.cublas,
              CUBLAS_OP_N, CUBLAS_OP_N,
              B.cols,     // m: 결과 행렬의 열 수 (row-major에서)
              A.rows,     // n: 결과 행렬의 행 수 (row-major에서)
              A.cols,     // k: 내부 차원 (A의 열 = B의 행)
              &alpha,
              d_B, MAX_DIM,   // B^T (cuBLAS 시점), leading dim = MAX_DIM
              d_A, MAX_DIM,   // A^T (cuBLAS 시점), leading dim = MAX_DIM
              &beta,
              d_C, MAX_DIM);  // C^T (cuBLAS 시점), leading dim = MAX_DIM
...
// ============================================================
// 7. Relaxation 함수 (CPU - 분기문이 많아 GPU 부적합)
// ============================================================
...
// 신경망 정방향 통과 (CPU - 벡터 연산은 GPU 오버헤드 > 이득)
...
// ============================================================
// 9. CROWN 알고리즘 (Forward/Backward Bound)
//    알고리즘 로직은 my_lirpa.cu와 완전히 동일
//    내부에서 호출하는 matmul, mat_add 등이 최적화됨
// ============================================================
...
BackwardBoundResult lirpa_backward_bound(const FullyConnectedNetwork &net,
  ...
  lower_M = output_lower_M ? *output_lower_M : make_eye(output_dim);
  upper_M = output_upper_M ? *output_upper_M : make_eye(output_dim);
  lower_p = output_lower_p ? *output_lower_p : make_zero_vector(lower_M.rows);
  upper_p = output_upper_p ? *output_upper_p : make_zero_vector(upper_M.rows);
...
// ============================================================
// 10. LiRPABackwardOnly 클래스 (my_lirpa.cu와 동일)
// ============================================================
