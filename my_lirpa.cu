...
namespace {
    ...
    __global__ void affine_min_gpu(const Matrix* A, const Vector* c, const Vector* xl, const Vector* xu, Vector* out) {
        int tid = blockDim.x * blockIdx.x + threadIdx.x;
        ...
        if (tid < A->rows) {
            ...
            for (int j = 0; j < A->cols; ++j) {
                ...
                sum += (aij > 0.0 ? aij : 0.0) * xl->v[j] + (aij < 0.0 ? aij : 0.0) * xu->v[j];
            }
            out->v[tid] = sum;
        }
    }
    ...
    Vector affine_min(const Matrix& A, const Vector& c, const Vector& x0, double eps) {
        ...
        affine_min_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_c, d_xl, d_xu, d_out);
        cudaDeviceSynchronize();
    ...
    __global__ void affine_max_gpu(const Matrix* A, const Vector* c, const Vector* xl, const Vector* xu, Vector* out) {
        int tid = blockDim.x * blockIdx.x + threadIdx.x;
        ...
        if (tid < A->rows) {
            ...
            for (int j = 0; j < A->cols; ++j) {
                double aij = A->a[tid][j];
                sum += (aij > 0.0 ? aij : 0.0) * xu->v[j] + (aij < 0.0 ? aij : 0.0) * xl->v[j];
            }
            out->v[tid] = sum;
        }
    }
    ...
    Vector affine_max(const Matrix& A, const Vector& c, const Vector& x0, double eps) {
        ...
        affine_max_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_c, d_xl, d_xu, d_out);
        cudaDeviceSynchronize();
    ...
    // ==================== relu_relax (GPU) ====================
    __global__ void relu_relax_gpu(const Vector* lower, const Vector* upper, Vector* alpha_l, Vector* beta_l, Vector* alpha_u, Vector* beta_u) {
        int tid = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    void relu_relax(const Vector& lower, const Vector& upper, Vector& alpha_l, Vector& beta_l, Vector& alpha_u, Vector& beta_u) {
        ...
        relu_relax_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_lower, d_upper, d_alpha_l, d_beta_l, d_alpha_u, d_beta_u);
        ...
        if (err != cudaSuccess) {
            std::cerr << "CUDA error in relu_relax memcpy back: " << cudaGetErrorString(err) << "\n";
        }
        ...
        // Debug
        // std::cerr << "DEBUG RELU_RELAX: lower=" << lower.v[0] << ", upper=" << upper.v[0] 
        ...
        for (int l = 0; l < net.num_layers; ++l) {
            ...
            for (int i = 0; i < s.n; ++i) {
                if (net.act[l] == ActivationType::Relu) {
                    s.v[i] = (s.v[i] > 0.0) ? s.v[i] : 0.0; // 간단한 relu
